...
#Bot for create user and a shared mailbox
class CreateSharedAndExchangeMailboxInAD(Bot):
    ...
    def execute(self,executionContext):
        try:
            newMailBox = executionContext["newMailBox"] 
            emailID = executionContext["emailID"] 
            password = executionContext["password"] 
            firstName = executionContext["firstName"]
            lastName = executionContext["lastName"]
            ...
            if not newMailBox:
                return {'Warning': 'newMailBox can not be empty'}
            if not emailID:
                return {'Warning': 'emailID can not be empty'}
            if not password:
                return {'Warning': 'password can not be empty'}
            if not firstName:
                return {'Warning': 'firstName can not be empty'}
            if not lastName:
                return {'Warning': 'lastName can not be empty'}
            ...
            previous_status,previous_output = self.executeServer(r'powershell "New-Mailbox -Shared -Name '+str(newMailBox)+ ' -firstNamePrincipalName ' + str(emailID) + ' -Password (ConvertTo-SecureString -String '+str(password)+' -AsPlainText -Force) -FirstName '+str(firstName)+' -LastName '+str(lastName) + r'"',str(varServerName),str(varUserName),str(varPassword))
            if previous_status != 'OK':
				return {'Validation Error' : 'Status failed'}
			else:
                return {'Status':'User and Shared Mailbox has been created'}
        except Exception as e:
            return {'Exception' : str(e)} 

if __name__ == "__main__":
    context = {}
    bot_obj = CreateSharedAndExchangeMailboxInAD()
    context = {'varServerName':'',varPassword':'','newMailBox':'','emailID':'','password':'','firstName':'','lastName':''}
    bot_obj.bot_init()
