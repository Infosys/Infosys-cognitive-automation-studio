...
class DeleteUserInWindowsAD(Bot):
	...
	def execute(self, executeContext):
		try:
			...
			previous_status,previous_output = self.executeServer(r'powershell "Get-ADUser -Identity '+str(varAdUsername)+r'"',
			...
			if var_check_ad_user and re.search('True',str(var_check_ad_user),re.IGNORECASE):
				...
				previous_status,previous_output = self.executeServer(r'powershell "Remove-ADUser -Identity '+
													str(varAdUsername)+r'"', str(varServerName),
													str(varUserName),str(varPassword))
				previous_status,previous_output = print(r'Deleted AD user sucessfully',r'IMP',r'NA')
...
if __name__ == '__main__':
	context = {}
	output = {}
	obj_snow = DeleteUserInWindowsAD()
