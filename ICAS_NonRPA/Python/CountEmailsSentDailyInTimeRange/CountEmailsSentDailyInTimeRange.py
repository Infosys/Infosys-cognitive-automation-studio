...
#the bot is to get the count of emails sent daily for a specified period
class CountEmailsSentDailyInTimeRange(Bot):
    ...
    def execute(self, executionContext):
        try:
            ...
            sentBox=outlook.GetdefaultFolder(5)    #5 is the index for the sentBox folder
            sentBoxMessages=sentBox.Items
            
            #Creating an Excel File
            writer=pandas.ExcelWriter(excelPath+'\\EmailsSentDailyReport.xlsx', engine='xlsxwriter')
            writer.save()            
            
            counter=0
            for singleDate in daterange(startDate, endDate):
                count=0
                for message in sentBoxMessages:
                ...
                dfExcel=pandas.read_excel(excelPath+'\\EmailsSentDailyReport.xlsx')
                result=pandas.concat([dfExcel, df], ignore_index=True)
                result.to_excel(excelPath+'\\EmailsSentDailyReport.xlsx', index=False)
                
                counter+=1
                print('Number of emails sent on '+str(singleDate)+': '+str(count))
...
if __name__=='__main__':
    context={}
    bot_obj=CountEmailsSentDailyInTimeRange()
