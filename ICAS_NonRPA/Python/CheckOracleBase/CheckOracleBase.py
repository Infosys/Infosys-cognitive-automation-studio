...
#            print('if exist "'+directory_path+'\\bin" echo exists')
            ...
            if sub_out.decode("ASCII")[:-2]=="exists":  # check if env variable value exists as a directory
                command = Session.run_command(shell,'if exist "'+directory_path+'\\admin" echo exists') 
                sub_out,sub_err,ret_code =Session.get_command_output(shell,command)
...
#                    print("Bin directory exists inside", str(sub_out))
                    ...
                    Session.cleanup_command(shell, command)
                    Session.close_shell(shell)
                    return("Invalid Oracle Base Directory (It is not an ORACLE_BASE directory)")
                    
            else:
                Session.cleanup_command(shell, command)
                Session.close_shell(shell)
                return("Invalid Oracle Base Directory (It is not a directory)")
                    
        else:
            Session.cleanup_command(shell, command)
            Session.close_shell(shell)
            return("ORACLE_BASE variable is not defined")                   
...
#                 return ("Missing argument :Command")
            ...
            if os=="linux" or os=="unix":
                envVarValue= self.getCmd_linux(serverName,userName,password,'echo $'+"ORACLE_BASE")
                if envVarValue is not None:
                    ...
                    if Dir_exists[:-1]=="exists":
                        Dir_exists= self.getCmd_linux(serverName,userName,password,'[ -d "'+ envVarValue[:-1] +'/admin'+'" ] && echo "exists"')
                        if Dir_exists is not None:#[:-1]=="exists":
                            return {'Status':'Success'}
                        else:
                            return {'Status':"Invalid Oracle Base Directory (It is not an ORACLE_BASE directory)"}
                    else:
                        return {'Status':"Invalid Oracle Base Directory (It is not a directory)"}
                else:
                    return {'Status':"ORACLE_BASE variable is not defined"}
                
#                out= self.getENV_linux(serverName,userName,password,'KAFKA_HOME')            
#                return {'Status':'Valid Directory: '+out}
            elif os=="windows":
                out= self.getENV_win(serverName,userName,password,'ORACLE_BASE')
                print(out)
                if out=="Success":
                    return {'Status':'Success'}
                else:
