...
class RevokeAccessFromPostgresDB(Bot):
	...
	def execute(self, executeContext) :
		try:
			...
			if not userName:
				return {'validation' :  'Enter a valid username for REVOKE permissions'}
		...
		try:
			cur = conn.cursor()
			sqlSelect = "REVOKE ALL PRIVILEGES ON DATABASE {0} FROM {1};".format(dbName,userName)
			print (sqlSelect)
...
if __name__ == '__main__':
	context = {}
	bot_obj = RevokeAccessFromPostgresDB()
