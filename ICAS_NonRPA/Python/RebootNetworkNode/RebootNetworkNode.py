...
# Arguments: ["string","varHost"],["string","varLocation"],["string","varCommand"],["password","varPassword"]



class RebootNetworkNode(Bot):
    ...
    def execute(self,executionContext):
        # Arguments: ["string","varHost"],["string","varLocation"],["string","varCommand"],["password","varPassword"]
        try:
            if "varHost" in executionContext:
                varHost = executionContext["varHost"]
            else:
                print('Missing argument: varHost','ERROR')
                return { 'output':'failure'}
            if "varLocation" in executionContext:
                varLocation = executionContext["varLocation"]
            else:
                print('Missing argument: varLocation','ERROR')
                return { 'output':'failure'}
            if "varCommand" in executionContext:
                varCommand = executionContext["varCommand"]
            else:
                print('Missing argument: varCommand','ERROR')
                return { 'output':'failure'}
            if "varPassword" in executionContext:
                varPassword = executionContext["varPassword"]
            else:
                print('Missing argument: varPassword','ERROR')
                return { 'output':'failure'}
            #previous_status,previous_output = connectNetwork(r'cisco_ios',str(varHost),22,r'sxkomaku',str(varPassword),r'undefined')
            try:
                network_connection = netmiko.ConnectHandler(device_type=r'cisco_ios',ip=str(varHost),port=22,username=r'sxkomaku',password=str(varPassword),secret=str(varPassword))
                # Not sure below about enable method is to exexute or not
		        #network_connection.enable()
                previous_status,previous_output =  'OK','Network connection made: '+str(varHost)
            ...
            if previous_output and re.search('"NOK"',str(previous_output),re.IGNORECASE):
                ...
                #previous_status,previous_output = connectServer(r'telnet',str(varHost),23,r'sxkomaku',str(varPassword))
                userName = r'sxkomaku'
                remote_connection = telnetlib.Telnet(str(varHost).lower(),23,timeout=server_timeout)
                if userName != 'NA':
                    td_out = remote_connection.read_until(b": ")
                    remote_connection.write((userName+"\n").encode('ascii'))
                    td_out = remote_connection.read_until(b"password: ")
                    remote_connection.write((str(varPassword)+"\n").encode('ascii'))
                td_out = remote_connection.read_until(b"ops_auto_telnet",server_timeout)
                

                #revious_output = executeServer(str(varCommand))
                remote_connection.write((str(varCommand)+"\n").encode('ascii'))
                return_text = remote_connection.read_until(b"ops_auto_telnet",telnet_interval)
                return_text = return_text.strip()

                
                var_comm_out = return_text
                #previous_status,previous_output = runScript(r'Network_File_MailIds.py',r'sequential',{"varLocation":"'+str(varLocation)+r'"},r'NA',r'NA')
                ...
                try:
                    ...
                    if user_team in team_names:
                        ...
                        user_team = 'NOK'
                    job_args = {"varLocation":"'+str(varLocation)+r'"}
                    ...
                    if userName != 'NA':
                        if pass_word == 'NA': 
                            #ret_status, pass_word = getLogins(userName)
                            return_text = 'NOK'
                            user_team = userTeam()
                            stack_list = inspect.stack()
                            user_team = "".join(instack[1].replace('\\','/').split('/')[-3] for instack in stack_list if 'Scripts' in instack[1])
                            ...
                            if os.path.exists(auto_db):
                                ...
                                db_cursor = db.execute("select access_key from user_list where user_team='"+user_team+"' and user_group='saccount' and userName=lower('"+str(userName)+"')")
                            ...
                            if not sql_out:
                                print('Logins not configured: '+str(userName),'WARN')
                                ret_status, pass_word = 'NOK',return_text
                            return_text = fernet.decryptString(sql_out[0])
                            print('Logins retrieved: '+str(userName),'INFO')
                            ret_status, pass_word = 'OK',return_text

                            if ret_status != 'OK':
                                print('NOK','Logins not found')
                                return { 'output':'failure'}
                        job_args['userName'] = userName
                ...
                #previous_status,previous_output = sendMail(str(var_comm_out),r'iops_auto@testexc.com',str(previous_output).split(','),r''+str(varHost)+r' - Info',r'mail.testexc.com',25,r'plain',r'NA',r'NA')
                try:
                    if pass_word == 'NA' and userName != 'NA':
                        ret_status, pass_word = getLogins(userName)
                        return_text = 'NOK'
                        user_team = userTeam()
                        stack_list = inspect.stack()
                ...
                #revious_status,previous_output = executeNetwork(str(varCommand),r'NO')
                if network_connection.check_config_mode():
                    network_connection.exit_config_mode()
                return_text = network_connection.send_command(str(varCommand))
                return_text = return_text.strip()
...
if __name__=="__main__":
    context={}
    bot_obj=RebootNetworkNode()
    
    context = {'varHost':'' ,'varLocation': '' , 'varCommand': '', 'varPassword':''}
