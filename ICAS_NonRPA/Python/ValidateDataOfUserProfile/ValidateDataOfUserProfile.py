...
#Bot to validate user properties in AD
class ValidateDataOfUserProfile(Bot):
    ...
    def execute(self,executionContext):
        try:
            ...
            previous_status,previous_output = self.executeServer(r'powershell "Get-ADUser-Filter*-Properties*'+ r'"',str(varServerName),str(varUserName),str(varPassword))
            if previous_status != 'OK':
				return {'Validation Error' : 'Status failed'}
			else:
...
if __name__ == "__main__":
    context = {}
    bot_obj = ValidateDataOfUserProfile()
