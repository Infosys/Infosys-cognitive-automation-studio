...
'''
# ------------------------------------------------------------------------------------------
# Arguments: ["string","username"],["password","password"]
# Sample Python Paramiko for Network Automation
# Written by anup.dusane. Downloaded from Automation Central
# ------------------------------------------------------------------------------------------
import paramiko
...
def timeStamp():
    """
    timeStamp function can be used for debugging or to display timestamp for specific event to a user
...
# ------------------------------------------------------------------------------------------
def searchPrompt(a_string, mode='cisco'):
    """
...
# ------------------------------------------------------------------------------------------
def sessionCLI(command_list, remote_host_ip, username='cisco', password='cisco', debug_flag=0, timeout=2, mode='cisco'):
    """
        ...
        while not prompt:
            while ssh_session.recv_ready():
                ...
                for line in stdout_temp.splitlines():
                    find_prompt = searchPrompt(line, mode=mode)
                    if find_prompt:
                        prompt = find_prompt
...
# ------------------------------------------------------------------------------------------
def deliverConfig(config, ip_addresses, username, password, mode='cisco'):
    log = ''
    ...
    for ip in ip_list:
        try:
            stdout, stderr, exit_status = sessionCLI(command_list, ip, username, password, debug_flag=0, mode=mode)
        except Exception as _:
        ...
        while not self.host_queue.empty():
            ...
            for cfg_block in self.config_list:
                ...
                for cfg in cfg_block['template']:
                    log += deliverConfig(cfg, host_ip, self.username, self.password, mode=self.mode)
...
def main(filename):
    ...
    print(timeStamp(), ': script started')
    task_queue = Queue()
    ...
    print(timeStamp(), ': script stopped')
# ------------------------------------------------------------------------------------------