...
#class for bot
class DeleteAccessOfServiceId(Bot):
    ...
    def execute(self,executionContext):
        try:
            ...
            serviceIdName=executionContext['serviceIdName']
            ...
            if  serviceIdName is None:
                return ("Missing argument :serviceIdName")
            ...
            stdin,stdout,stderr=ssh.exec_command('sudo userdel -r {}'.format(serviceIdName))
            outlines=stdout.readlines()
            Output=''.join(outlines)+'Service ID deleted Successfully'
            ssh.close()
...
if __name__ == "__main__":
    ...
    bot_obj = DeleteAccessOfServiceId()
    
    context = {'serverName':'','userName':'','password':'','serviceIdName':''}
    bot_obj.bot_init()
