...
@Bot(name = "CopyFolderToWindows", version = "1.0", description = "", botCategory = "String", author = "", technology = "", technologyCode = "01", botCategoryCode = "01", botId = "08")
...
public class CopyFolderToWindows {
	private static final Logger log = LoggerFactory.getLogger(CopyFolderToWindows.class);
	...
	public Status execute(@InputParameter(name = "Source", constraints = { EXISTS }) String source,
		...
		String creds = "$Username = '" + userName + "';$hostName='" + serverName + "';$PlainPassword ='" + password
				...
				+ SourcePath + "\" -Destination \"" + target + "\" -Recurse -ToSession $session;"
				+ "if($?){Write-Host \"True\"}Else {Write-Host \"False\"}";
		
		log.info("Copy Folder Details...." + SourcePath);
